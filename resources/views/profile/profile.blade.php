    ...
    @if(session('success'))
        <div x-data="{ open: true }" x-show="open" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">
            ...
            <span @click="open = false" class="absolute top-0 bottom-0 right-0 px-4 py-3 cursor-pointer">
                <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.303l-2.651 3.546a1.2 1.2 0 1 1-1.697-1.697l3.546-2.651-3.546-2.651a1.2 1.2 0 0 1 1.697-1.697l2.651 3.546 2.651-3.546a1.2 1.2 0 0 1 1.697 1.697l-3.546 2.651 3.546 2.651a1.2 1.2 0 0 1 0 1.697z"/></svg>
            </span>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-8" x-data="{ currentTab: 'profile' }">
        ...
        <div class="md:col-span-1 bg-white rounded-lg shadow-lg p-6 h-fit sticky top-24">
            <div class="text-center mb-6">
                <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : 'https://via.placeholder.com/128/000000/FFFFFF?text=A' }}" 
                        alt="{{ $user->name }}" 
                        class="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-4 border-gray-100 shadow-md">
                <h2 class="text-xl font-black text-gray-900 uppercase">{{ $user->name }}</h2>
        ...
        <div class="md:col-span-3 space-y-8">
            ...
            <div x-show="currentTab === 'profile'" class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h2 class="text-lg font-black text-gray-900 mb-4 tracking-wide uppercase">Change Avatar</h2>
                    
                    <form action="{{ route('profile.avatar') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div class="flex items-center space-x-4">
                            <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : 'https://via.placeholder.com/64/000000/FFFFFF?text=A' }}" 
                                class="w-16 h-16 rounded-full object-cover border" alt="Current Avatar">
                            
                            <input type="file" name="avatar" required
                                    class="form-input block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none p-2">
                        </div>
                        @error('avatar')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        <button type="submit" class="w-full bg-black text-white py-2 rounded font-bold hover:bg-gray-800 transition uppercase text-sm">
                            Upload New Avatar
                        </button>
                    </form>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" class="space-y-4">
                        ...
                        {{-- START: SỬA LỖI PHP 500 (Only variables should be passed by reference) --}}
                        @php
                            $nameParts = explode(' ', $user->name);
                            // Lấy từ đầu tiên là First Name
                            $firstName = array_shift($nameParts); 
                            // Các từ còn lại là Last Name
                    ...
                    @forelse($orders as $order)
                        <div class="border-b border-gray-200 py-4 last:border-b-0">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                @php
                                    // Sử dụng trim(strtolower()) để tránh lỗi không tìm thấy key nếu status có khoảng trắng hoặc chữ hoa
                            ...
                            <div class="flex justify-between items-center">
                                {{-- Cần đảm bảo Model Order có thuộc tính items_count hoặc total --}}
